		...
		switch (keycode) {
			...
			case E.KEY_LEFT:
				if (!popupvisible && this._menuitemindex <= -1) {
					...
					if (this._menuitemindex > -1 && nexacro._isNull(this._popupmenu)) {
						previouse_item_index = this._menuitemindex;
						this._select_menuitem(this._getItemIndex(-1), previouse_item_index, true, "prev");
					}
					else {
						this._fire_on_Popupmenu(keycode, alt_key, ctrl_key, shift_key, fire_comp, refer_comp);
					}
			...
			case E.KEY_RIGHT:
				if (!popupvisible && this._menuitemindex <= -1) {
					...
					if (this._menuitemindex > -1 && (nexacro._isNull(this._popupmenu) || !menuexpand)) {
						previouse_item_index = this._menuitemindex;
						this._select_menuitem(this._getItemIndex(1), previouse_item_index, true, "next");
					}
					else {
						this._fire_on_Popupmenu(keycode, alt_key, ctrl_key, shift_key, fire_comp, refer_comp);
					}
